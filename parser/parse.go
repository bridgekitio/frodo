...
func registerTypeEntry(ctx *Context, registry TypeRegistry, entry *TypeDeclaration, t types.Type) error {
	...
	case *types.Named:
		...
		entry.Implements.MarshalJSON = implements.Method(tt, "MarshalJSON", nil, []string{"[]byte", "error"})
		entry.Implements.ContentGetter = implements.Method(tt, "Content", nil, []string{"io.ReadCloser"})
...
func ApplyFunctionDocumentation(ctx *Context, function *ServiceFunctionDeclaration) {
	...
	// By default, this endpoint includes an HTTP/API route using RPC-style paths. Event
	// routes are added on-demand when we encounter the "ON xxx.yyy" doc option.
	apiRoute := GatewayRoute{
		Function:    function,
		GatewayType: "API",
		Method:      "POST",
	...
	for _, line := range ctx.Documentation.ForFunction(function) {
		...
		case strings.HasPrefix(line, "GET "):
			...
			apiRoute.Path = normalizePath(line[4:])
		case strings.HasPrefix(line, "PUT "):
			apiRoute.Method = http.MethodPut
			apiRoute.Path = normalizePath(line[4:])
		case strings.HasPrefix(line, "POST "):
			apiRoute.Method = http.MethodPost
			apiRoute.Path = normalizePath(line[5:])
		case strings.HasPrefix(line, "PATCH "):
			apiRoute.Method = http.MethodPatch
			apiRoute.Path = normalizePath(line[6:])
		case strings.HasPrefix(line, "DELETE "):
			apiRoute.Method = http.MethodDelete
			apiRoute.Path = normalizePath(line[7:])
