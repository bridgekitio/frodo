...
func (suite *ParserSuite) TestBasic() {
	...
	suite.assertFunction(service, "Bowl", expectedFunction{
		...
		Routes: parser.GatewayRoutes{
			&parser.GatewayRoute{GatewayType: "API", Method: "POST", Path: "/DudeService.Bowl", Status: 200},
		},
	})
...
func (suite *ParserSuite) TestDocOptions() {
	...
	suite.assertFunction(service, "Walter", expectedFunction{
		Documentation: parser.DocumentationLines{},
		Routes: parser.GatewayRoutes{
			&parser.GatewayRoute{GatewayType: "API", Method: "POST", Path: "/LebowskiService.Walter", Status: 200},
		},
	})
	...
	suite.assertFunction(service, "Donny", expectedFunction{
		Documentation: parser.DocumentationLines{},
		Routes: parser.GatewayRoutes{
			&parser.GatewayRoute{GatewayType: "API", Method: "POST", Path: "/LebowskiService.Donny", Status: 204},
		},
	})
	...
	suite.assertFunction(service, "Jackie", expectedFunction{
		Documentation: parser.DocumentationLines{},
		Routes: parser.GatewayRoutes{
			&parser.GatewayRoute{GatewayType: "API", Method: "PUT", Path: "/dude/jail", Status: 200},
		},
	})
	...
	suite.assertFunction(service, "Rug", expectedFunction{
		...
		Routes: parser.GatewayRoutes{
			&parser.GatewayRoute{GatewayType: "API", Method: "HEAD", Path: "/ties/room/together", Status: 200},
		},
	})

	suite.assertFunction(service, "Bunny", expectedFunction{
		Documentation: parser.DocumentationLines{
			"Bunny should be omitted.",
		},
		Routes: parser.GatewayRoutes{},
	})

	suite.assertFunction(service, "Bowling", expectedFunction{
		Documentation: parser.DocumentationLines{
			"Bowling : Fuck it, dude. Let's go bowling.",
		},
		Routes: parser.GatewayRoutes{
			// Even though we only specified events, you still get the default API endpoint.
			&parser.GatewayRoute{GatewayType: "API", Method: "POST", Path: "/LebowskiService.Bowling", Status: 200},
			&parser.GatewayRoute{GatewayType: "EVENTS", Method: "ON", Path: "LebowskiService.Dude"},
			&parser.GatewayRoute{GatewayType: "EVENTS", Method: "ON", Path: "LebowskiService.Walter"},
			&parser.GatewayRoute{GatewayType: "EVENTS", Method: "ON", Path: "LebowskiService.Donny"},
		},
	})

	suite.assertFunction(service, "BowlingEnd", expectedFunction{
		Documentation: parser.DocumentationLines{
			"BowlingEnd doesn't include Donny (tears...) or a direct API endpoint.",
		},
		Routes: parser.GatewayRoutes{
			&parser.GatewayRoute{GatewayType: "EVENTS", Method: "ON", Path: "LebowskiService.Dude"},
			&parser.GatewayRoute{GatewayType: "EVENTS", Method: "ON", Path: "LebowskiService.Walter"},
...
func (suite *ParserSuite) TestFieldTypes() {
	...
	suite.assertFieldType(fields, "Interface", expectedFieldType{Name: "any", Pointer: false, JSON: "object"})
...
func (suite *ParserSuite) assertFunction(service *parser.ServiceDeclaration, functionName string, expected expectedFunction) {
	...
	apiRoute := f.Routes.API()
	switch expectedRoute := expected.Routes.API(); expectedRoute {
	case nil:
		suite.Require().Nil(apiRoute, "%s: API Route: Should not exist", name)
	default:
		suite.Require().NotNil(apiRoute, "%s: API Route: Not found", name)
		suite.Equal(f, apiRoute.Function, "%s: API Route: Incorrect function back-pointer", name)
		suite.Equal(expectedRoute.Path, apiRoute.Path, "%s: API Route: Incorrect path", name)
		suite.Equal(expectedRoute.Method, apiRoute.Method, "%s: API Route: Incorrect method", name)
		suite.Equal(expectedRoute.Status, apiRoute.Status, "%s: API Route: Incorrect status", name)
	}

	events := f.Routes.Events()
	expectedEvents := expected.Routes.Events()
	suite.Require().Equal(len(expectedEvents), len(events), "%s: Event Route: Incorrect number of events", name)
	for i, expectedEvent := range expected.Routes.Events() {
		suite.Require().NotNil(events[i], "%s: Event Route: Not found", name)
		suite.Equal(f, events[i].Function, "%s: Event Route: Incorrect function back-pointer", name)
		suite.Equal(expectedEvent.Path, events[i].Path, "%s: Event Route: Incorrect path", name)
		suite.Equal(expectedEvent.Method, events[i].Method, "%s: Event Route: Incorrect method", name)
	}
